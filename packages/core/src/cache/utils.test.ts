...
describe('isEntityLinkArray', () => {
  it('should return false for empty array', () => {
    expect(isEntityLinkArray([])).toBe(false);
  });

  it('should return true for entity link array', () => {
    expect(isEntityLinkArray([{ [EntityLinkKey]: 'User:1' }, { [EntityLinkKey]: 'User:2' }])).toBe(true);
  });

  it('should return false for scalar string array', () => {
    expect(isEntityLinkArray(['a', 'b'])).toBe(false);
  });

  it('should return false for number array', () => {
    expect(isEntityLinkArray([1, 2, 3])).toBe(false);
  });

  it('should return false for boolean array', () => {
    expect(isEntityLinkArray([true, false])).toBe(false);
  });

  it('should return true for null-containing entity array', () => {
    expect(isEntityLinkArray([null, { [EntityLinkKey]: 'User:1' }, null])).toBe(true);
  });

  it('should return false for all-null array', () => {
    expect(isEntityLinkArray([null, null])).toBe(false);
  });

  it('should return true for nested entity link array', () => {
    expect(isEntityLinkArray([[{ [EntityLinkKey]: 'User:1' }]])).toBe(true);
  });

  it('should return true for double-nested entity link array', () => {
    expect(isEntityLinkArray([[[{ [EntityLinkKey]: 'User:1' }]]])).toBe(true);
  });

  it('should return false for non-entity object array', () => {
    expect(isEntityLinkArray([{ name: 'foo' }])).toBe(false);
  });

  it('should return true for single entity link element', () => {
    expect(isEntityLinkArray([{ [EntityLinkKey]: 'User:1' }])).toBe(true);
  });

  it('should return true for undefined-containing entity array', () => {
    expect(isEntityLinkArray([undefined, { [EntityLinkKey]: 'User:1' }])).toBe(true);
  });

  it('should return true for mixed array when first non-null item is entity link', () => {
    expect(isEntityLinkArray([{ [EntityLinkKey]: 'User:1' }, 'string', 42])).toBe(true);
  });
});

describe('isEntityLinkArrayEqual', () => {
  it('should return true for same keys in same order', () => {
    expect(
      isEntityLinkArrayEqual(
        [{ [EntityLinkKey]: 'User:1' }, { [EntityLinkKey]: 'User:2' }],
        [{ [EntityLinkKey]: 'User:1' }, { [EntityLinkKey]: 'User:2' }],
      ),
    ).toBe(true);
  });
  ...
  it('should return false for different keys', () => {
    expect(isEntityLinkArrayEqual([{ [EntityLinkKey]: 'User:1' }], [{ [EntityLinkKey]: 'User:2' }])).toBe(false);
  });
  ...
  it('should return true for both empty', () => {
    expect(isEntityLinkArrayEqual([], [])).toBe(true);
  });

  it('should return true for same null slots', () => {
    expect(isEntityLinkArrayEqual([null, { [EntityLinkKey]: 'User:1' }], [null, { [EntityLinkKey]: 'User:1' }])).toBe(
      true,
    );
  });

  it('should return false for null vs non-null', () => {
    expect(isEntityLinkArrayEqual([null], [{ [EntityLinkKey]: 'User:1' }])).toBe(false);
  });
  ...
  it('should return false for different order', () => {
    expect(
      isEntityLinkArrayEqual(
        [{ [EntityLinkKey]: 'User:1' }, { [EntityLinkKey]: 'User:2' }],
        [{ [EntityLinkKey]: 'User:2' }, { [EntityLinkKey]: 'User:1' }],
      ),
    ).toBe(false);
  });

  it('should return false for one empty one not', () => {
    expect(isEntityLinkArrayEqual([], [{ [EntityLinkKey]: 'User:1' }])).toBe(false);
  });
});
